class NumberConverter
  def convert(number)
    ...
    if digits.count == 3 && (digits[1]).zero? && (digits[2]).zero?
      ...
      last_digit = digits[2]
      "one hundred and #{number_in_words[last_digit]}"
    elsif number.between?(110, 119)
      last_two_digits = digits.drop(1).join.to_i
      "one hundred and #{number_in_words[last_two_digits]}"
    elsif digits.count == 3
      first_digit = digits[0]
      second_digit = digits[1]
      third_digit = digits[2]
      fourth_word = second_digit.zero? ? '' : "#{number_in_words[second_digit * 10]} "
      "#{number_in_words[first_digit]} hundred and #{fourth_word}#{number_in_words[third_digit]}"
